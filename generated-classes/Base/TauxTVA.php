...
/**
 * Base class that represents a row from the 'tauxTVA' table.
 *
 *
 *
 * @package    propel.generator..Base
 */
abstract class TauxTVA implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\TauxTVATableMap';
    ...
    /**
     * Initializes internal state of Base\TauxTVA object.
     */
    ...
    /**
     * Compares this with another <code>TauxTVA</code> instance.  If
     * <code>obj</code> is an instance of <code>TauxTVA</code>, delegates to
     * <code>equals(TauxTVA)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|TauxTVA The current object, for fluid interface
     */
     ...
     * @return $this|\TauxTVA The current object (for fluent API support)
     */
    public function setIdTaux($v)
    {
        ...
        if ($this->idtaux !== $v) {
            $this->idtaux = $v;
            $this->modifiedColumns[TauxTVATableMap::COL_IDTAUX] = true;
        }
    ...
    /**
     * Set the value of [taux] column.
     *
     * @param double $v new value
     * @return $this|\TauxTVA The current object (for fluent API support)
     */
    public function setTaux($v)
    {
        if ($v !== null) {
            $v = (double) $v;
        }
        ...
        if ($this->taux !== $v) {
            $this->taux = $v;
            $this->modifiedColumns[TauxTVATableMap::COL_TAUX] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TauxTVATableMap::translateFieldName('IdTaux', TableMap::TYPE_PHPNAME, $indexType)];
            $this->idtaux = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TauxTVATableMap::translateFieldName('Taux', TableMap::TYPE_PHPNAME, $indexType)];
            $this->taux = (null !== $col) ? (double) $col : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = TauxTVATableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\TauxTVA'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildTauxTVAQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see TauxTVA::setDeleted()
     * @see TauxTVA::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildTauxTVAQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                TauxTVATableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TauxTVATableMap::COL_IDTAUX)) {
            $modifiedColumns[':p' . $index++]  = 'idTaux';
        }
        if ($this->isColumnModified(TauxTVATableMap::COL_TAUX)) {
            $modifiedColumns[':p' . $index++]  = 'taux';
        }

        $sql = sprintf(
            'INSERT INTO tauxTVA (%s) VALUES (%s)',
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'idTaux':
                        $stmt->bindValue($identifier, $this->idtaux, PDO::PARAM_INT);
                        break;
                    case 'taux':
                        $stmt->bindValue($identifier, $this->taux, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    {
        $pos = TauxTVATableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['TauxTVA'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['TauxTVA'][$this->hashCode()] = true;
        $keys = TauxTVATableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getIdTaux(),
            $keys[1] => $this->getTaux(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setIdTaux($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setTaux($arr[$keys[1]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TauxTVATableMap::COL_IDTAUX)) {
            $criteria->add(TauxTVATableMap::COL_IDTAUX, $this->idtaux);
        }
        if ($this->isColumnModified(TauxTVATableMap::COL_TAUX)) {
            $criteria->add(TauxTVATableMap::COL_TAUX, $this->taux);
        }
    ...
    {
        $criteria = ChildTauxTVAQuery::create();
        $criteria->add(TauxTVATableMap::COL_IDTAUX, $this->idtaux);
    ...
    /**
     * Generic method to set the primary key (idtaux column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \TauxTVA (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setIdTaux(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \TauxTVA Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildTauxTVA is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildTauxTVA The current object (for fluent API support)
     */
    public function setArticles(Collection $articles, ConnectionInterface $con = null)
    {
        ...
        foreach ($articlesToDelete as $articleRemoved) {
            $articleRemoved->setTauxTVA(null);
        }
     ...
     * Otherwise if this TauxTVA is new, it will return
     * an empty collection; or if this TauxTVA has previously
     * been saved, it will retrieve related Articles from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in TauxTVA.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
    ...
    {
        return (string) $this->exportTo(TauxTVATableMap::DEFAULT_STRING_FORMAT);
    }
